...
## Annotate
	...
	$R -e 'library(AnchoredFusion);annovar.exon.cds.extraction(input = "__breakpoint.annotated.hg19_multianno.txt")' > /dev/null 2>&1
    fi
...
##=== anno middle split
	if [ -s split.mid ]; then
		...
		if [ $AnnotationMethod = "annovar" ]; then
		    ...
		    $R -e 'library(AnchoredFusion);annovar.exon.cds.extraction(input = "mid.anno.hg19_multianno.txt")' > /dev/null 2>&1
		fi
