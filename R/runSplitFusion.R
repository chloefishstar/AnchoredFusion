# 
runSplitFusion <- function(configFile){
...
##==== Step 1: fastq to consolidated bam (de-duplication based on unique UMI and adatptor ligation site) ====
	...
	if ("1_fastq-bam" %in% unlist(strsplit(steps, ","))){
		suppressMessages(system(paste0("bash ", SplitFusionPath, "/exec/SplitFusion.1_fastq-bam.sh ",configFile, " SA")))
...
##==== 2: consolidated.bam to breakpoint candidates (no filter) ====
	...
	if ("2_bam-breakpoint" %in% unlist(strsplit(steps, ","))) {
		suppressMessages(system(paste0("bash ", SplitFusionPath, "/exec/SplitFusion.2_bam-breakpoint.sh ",configFile)))
...
##==== 3: filters ====
	...
	if ("3_breakpoint-filter" %in% unlist(strsplit(steps, ","))) {
		suppressMessages(system(paste0("bash ", SplitFusionPath, "/exec/SplitFusion.3_breakpoint-filter.sh ",configFile)))
...
##==== 4: Annotate breakpoint gene, exon, cDNA position
	...
	if ("4_breakpoint-anno" %in% unlist(strsplit(steps, ","))) {
        	suppressMessages(system(paste0("bash ", SplitFusionPath, "/exec/SplitFusion.4_breakpoint-anno.sh ",configFile)))
...
##==== 5: further processing (in-frame status determination, etc)
	...
	if ( "5_breakpoint-anno-post" %in% unlist(strsplit(steps, ","))) {
		suppressMessages(SplitFusion.breakpoint.anno.postscript(configFile = configFile))
